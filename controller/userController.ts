...
import { createprofileService, verifyemailService,getdataService } from '../service/userService';
interface type {
    status:string,
    msg:string,
    data?:any
...
export const createprofileController:RequestHandler=async(req:Request,res:Response)=>{
    let response :type = {
     ...
     try{
        ...
        const querys=await getdataService(Pool,data);
...
export const verifyemailController:RequestHandler=async(req:Request,res:Response)=>{
    let response :type = {
     ...
     try{
        const datas:any=req.body;
        const query:any=await getdataService(Pool,datas)
